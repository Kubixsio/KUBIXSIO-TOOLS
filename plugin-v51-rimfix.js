function runRimLight(){
  ...
  postScript('('+ktxCreateRimLightV51.toString()+')();');
}

function ktxCreateRimLightV51(){
  ...
  function createBaseOutline(){
    ...
    try{
      doc.selection.stroke(c,BASE_STROKE,StrokeLocation.OUTSIDE,ColorBlendMode.NORMAL,100,false);
      ok=true;
    }catch(e){
      try{doc.selection.stroke(c,BASE_STROKE);ok=true}catch(e2){ok=false}
    }
    if(ok){
      try{
        loadAlpha(source);
    ...
    makeRim();
    if(!loadAlpha(source))return false;
    if(!expandSelection(BASE_STROKE))return false;
    doc.activeLayer=rim;
    try{doc.selection.fill(c,ColorBlendMode.NORMAL,100,false)}catch(e4){try{doc.selection.fill(c)}catch(e5){return false}}
    if(!loadAlpha(source))return false;
    doc.activeLayer=rim;
    try{doc.selection.clear();doc.selection.deselect()}catch(e6){return false}
    return !!box(rim);
  }
  function makeSolidWhite(){
    if(!loadAlpha(rim))return false;
    if(!expandSelection(EXPAND_BY))return false;
    doc.activeLayer=rim;
    var c=white();
    for(var i=0;i<8;i++){
      try{doc.selection.fill(c,ColorBlendMode.NORMAL,100,false)}catch(e){
        try{doc.selection.fill(c)}catch(e2){return false}
      }
    }
  ...
  function build(){
    ...
    if(!createBaseOutline())return 'Nie udało się utworzyć bazowego obrysu';

    stage='solid white failed';
    if(!makeSolidWhite())return 'Nie udało się zamienić obrysu na pełną biel';
  ...
  if(!error){
    app.echoToOE('KTX_OK|RIM LIGHT gotowy • solidna biel • około 2 px • nad renderem');
  }else{
    var cleanup=[];
